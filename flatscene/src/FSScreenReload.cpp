...
void FSScreen::saveResources(GraphicResources &info) {
	...
	for (map<int,FSWriter::WriterImpl::SFont*>::iterator it=writer._impl->Fonts.begin();it!=writer._impl->Fonts.end();++it) {
	...
	for (map<FSEngine*,FSWriter::WriterImpl::SData*>::iterator it=writer._impl->session.begin();it!=writer._impl->session.end();++it) {
