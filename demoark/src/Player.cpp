...
void CPlayer::init(std::list<std::string>& activationIds, int v) {
#ifdef LOG_JUGADORES
...
void CPlayer::blockFutureActionCandidates() {
    ...
    if (act) {
        ...
        if (actMov) {
            std::vector<CAction*>* actionBrothers = actMov->getBrothers();
