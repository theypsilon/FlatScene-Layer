...
bool CActionGraph::add(TiXmlElement *newNode) {
    ...
    if ( parentNode && parentNode->Attribute("type") && strcmp(parentNode->Value(),"action")==0 ) {
    	...
    	if (tipoActual!=-1) {
    		...
    		FSLib.Error(string("Tipo de nodo ya usado o indefinido... type:")+parentNode->Attribute("type"));
    	}
...
bool CActionGraph::mix() {
    ...
    for (list<COrder*>::iterator it=nodos.begin();it!=nodos.end();++it) {		// Por cada acci�n
