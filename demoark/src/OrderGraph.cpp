...
CEnlace::CEnlace(std::string referencia,std::string tecla,std::string evento) : referencia(referencia), tecla(tecla), evento(evento) {}
CEnlace::~CEnlace() {}

COrder::COrder(CAction* n, std::string identidad) : nodo(n), identidad(identidad), prioridad(-1) {}
COrder::~COrder() {}
...
CActionGraph::~CActionGraph() {
    freemem(tipos);
    for (std::list<COrder*>::iterator it=nodos.begin();it!=nodos.end();++it) {
//      delete (*it)->nodo;
        delete (*it);
    }
...
COrder* CActionGraph::createNode(const char *c, TiXmlElement *newNode,const char* s) {
    if (!newNode->Attribute("id")) FSLib.Error("Acci�n sin identificador imposible de crear.");
    COrder* newInstance = NULL;
    std::string name = (s==NULL)? std::string(c) : std::string(c)+s;
    if (strcmp(c,"quiet")==0) {
        ...
        FSLib.Error("Se ha intentado agregar un tipo de acci�n restringido.");
    }
    return newInstance;
}
...
bool CActionGraph::add(TiXmlElement *newNode) {
    ...
    if ( parentNode && parentNode->Attribute("type") && strcmp(parentNode->Value(),"action")==0 ) {
        ...
        if (tipoActual!=-1) {
            ...
            if (hijo) {
                        ...
                        while (hijo && strcmp(hijo->Value(),"node")==0) {
                            ...
                            while (enlace && enlace->Attribute("ref") && enlace->Attribute("event")) {
                                std::string ref(enlace->Attribute("ref"));
                                if (enlace->Attribute("subnode"))
                                    ref = ref+"/"+enlace->Attribute("subnode");
                                std::string key="0";
                                if (enlace->Attribute("key"))
                                    key=enlace->Attribute("key");
                                newAccion->enlaces.push_back(CEnlace(ref,key,std::string(enlace->Attribute("event"))));
                            ...
                            while (enlace && strcmp(enlace->Value(),"family")==0 && enlace->Attribute("id") && enlace->Attribute("member") ) {
                                std::string familyName(enlace->Attribute("id"));
                                ...
                                if (mapaDeFamilias.find(familyName)!=mapaDeFamilias.end() && (mapaDeFamilias[familyName]).find(miembro)!= (mapaDeFamilias[familyName]).end())
                                    FSLib.Error("En cada familia de acciones, no puede haber m�s de una acci�n con el mismo puesto de miembro.");
                        ...
                        while (hijo && strcmp(hijo->Value(),"link")==0 && hijo->Attribute("ref") && hijo->Attribute("event") ) {
                            std::string ref(hijo->Attribute("ref"));
                            ...
                            for (std::list<COrder*>::iterator it=nodosHijo.begin();it!=nodosHijo.end();++it) {
                                if (!ignoreself || strcmp((*it)->identidad.c_str(),ref.c_str())!=0)
                                    (*it)->enlaces.push_back(CEnlace(ref,key,std::string(hijo->Attribute("event"))));
                            }
                            hijo = hijo->NextSiblingElement();
                        }
                        for (std::list<COrder*>::iterator it=nodosHijo.begin();it!=nodosHijo.end();++it)
                        ...
                        while (hijo && strcmp(hijo->Value(),"link")==0 && hijo->Attribute("ref") && hijo->Attribute("event")) {
                            ...
                            if (!ignoreself || strcmp(newAccion->identidad.c_str(),ref.c_str())!=0)
                                newAccion->enlaces.push_back(CEnlace(ref,key,std::string(hijo->Attribute("event"))));
                            hijo = hijo->NextSiblingElement();
                        }
                        hijo= newNode->FirstChildElement("family");
                        while (hijo && hijo->Attribute("id") && hijo->Attribute("member") ) {
                            std::string familyName(hijo->Attribute("id"));
            ...
            FSLib.Error(std::string("Tipo de nodo ya usado o indefinido... type:")+parentNode->Attribute("type"));
        }
...
bool CActionGraph::mix() {
    ...
    for (std::list<COrder*>::iterator it=nodos.begin();it!=nodos.end();++it) {
        ...
        for (std::list<CEnlace>::iterator kt=(*it)->enlaces.begin();kt!=(*it)->enlaces.end();++kt) {
    ...
    for (std::list<COrder*>::iterator it=nodos.begin();it!=nodos.end();++it) {       // Por cada acci�n
        for (std::list<CEnlace>::iterator kt=(*it)->enlaces.begin();kt!=(*it)->enlaces.end();++kt) { // Recorremos todos sus enlaces
            if (strcmp((*kt).evento.c_str(),"onkeydown")==0) { // Si es un enlace de tipo onkeydown...
                int tecla = getKey((*kt).tecla.c_str());    // Miramos que tecla tiene (lugar que ocupar�a en el array de keydowns de la acci�n)

                CAction* n = (CAction*)search((*kt).referencia.c_str());    // Obtenemos la acci�n que corresponde a dicha referencia.
                if (tecla==-1 || !n) {
                    printf("\n%s\n",(*kt).referencia.c_str());
                    FSLib.Error("\nFallo en dependencias en la fase de mezcla del grafo de acciones.");
                }
                ((CAction*)((*it)->nodo))->setKeydown(tecla,n); // Enlazamos la acci�n referenciada a la acci�n de esta iteraci�n para 'it'.
                ...
                FSLib.Error("\nA�n no se ha implementado funcionalidad para el evento 'onrepeat'.");
            }
        }
        if ((*it)->prioridad > -1) {
            ((CAction*)(*it)->nodo)->setPriority((*it)->prioridad);     // Asignamos la prioridad que le toca a la acci�n (si no lo hacemos, ser� 0).
        }
    }
    std::vector<CAction*>* arrayArrows;

    for (TypeFamily::iterator it =mapaDeFamilias.begin();it!=mapaDeFamilias.end();++it) {
        arrayArrows = new std::vector<CAction*>;
...
std::string CActionGraph::sinBarra(const char* c) {
    int i = 0;
    std::string ret="";
    while (c[i]!='/' && c[i]!='\0') {
        ret += c[i];
...
CGarbageCollectorOfActions* CActionGraph::createGarbageCollector() {
    ...
    for (std::list<COrder*>::iterator it=nodos.begin();it!=nodos.end();++it)
        garbage->add((*it)->nodo);
    /*
    for (std::list<CAction**>::iterator it=listOfBrothers.begin();it!=listOfBrothers.end();++it)
